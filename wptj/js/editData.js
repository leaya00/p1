...
Ext.onReady(function () {
	myMask = new Ext.LoadMask($('body').get(0), { msg: "Please wait..." });
    //    myMask.show();
    //       myMask.hide();
	LoadUI();
	LoadGrid();
    ...
    shopStore = Ext.create('Ext.data.Store',
       {
           ...
           proxy: {
               ...
               extraParams: { op:'select',type:'shop' }
           },
           model: 'dictModel',
           autoLoad: true
           //remoteFilter: true
	...
	objectStore = Ext.create('Ext.data.Store',
       {
           ...
           proxy: {
               ...
               extraParams: { op:'select',type:'object' }
           },
           model: 'dictModel',
           autoLoad: true
...
LoadGrid = function () {
    ...
    Ext.define('gridModel', {
        extend: 'Ext.data.Model',
        fields: ['id', 'sdate', 'edate','price','shop','object','createname','shop_s','object_s']
    });
           ...
           listeners: {
               'load': function (s, records, successful, eOpts) {
                   ...
                   if (records.length > 0) {
                       ...
                       dataGrid.fireEventArgs("itemclick", [null, records[0], null, 0]);
                   } else {
                       Clear_form();
                       ext_btnAdd.setDisabled(false);
                       ext_btnModify.setDisabled(true);
...
del = function () {
    Ext.MessageBox.confirm('确认删除', '您确认要删除此条信息?', function (re) {
        ...
        if (re == "yes") {
            $.ajax({
                type: "POST",url: "./json/wptj_data.php?op=delete",
                data: { id: $("#hid_id").val() },
                success: function (msg) {
                    if (msg.result==true) {
                        gridStore.reload();
